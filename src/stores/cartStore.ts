...
export function buildLineKey(variantId: string, attributes: CartAttribute[] = []) {
  const attrs = [...attributes]
    .filter((a) => a.value?.trim())
    .sort((a, b) => a.key.localeCompare(b.key))
    .map((a) => `${a.key}=${a.value.trim()}`)
    .join("|");
  return attrs ? `${variantId}::${attrs}` : variantId;
}

function cleanAttributes(attributes: CartAttribute[] = []) {
  return attributes
    .filter((a) => a.key?.trim() && a.value?.trim())
    .map((a) => ({ key: a.key.trim(), value: a.value.trim().slice(0, 250) }));
}
  ...
  mutation cartCreate($input: CartInput!) {
    cartCreate(input: $input) {
      cart {
        id
        checkoutUrl
        lines(first: 100) { edges { node { ${LINE_FIELDS} } } }
      }
...
function matchLine(edges: any[], item: CartItem) {
  return edges.find(
    (l: any) =>
      l.node.merchandise?.id === item.variantId &&
      buildLineKey(l.node.merchandise.id, l.node.attributes || []) === item.lineKey,
  );
}
      ...
      lines: [
        {
          ...
          attributes: cleanAttributes(item.attributes),
        },
      ],
    },
  });
...
async function addLineToShopifyCart(cartId: string, item: CartItem) {
  ...
  const newLine = matchLine(lines, item);
...
export const useCartStore = create<CartStore>()(
  persist(
    (set, get) => ({
      ...
      addItem: async (input) => {
        ...
        const item: CartItem = {
          ...
          attributes,
          lineId: null,
          lineKey: buildLineKey(input.variantId, attributes),
        };
        const { items, cartId, clearCart } = get();
        const existingItem = items.find((i) => i.lineKey === item.lineKey);
    ...
    }),
    {
      name: "shopify-cart",
      version: 2,
      migrate: () => ({ items: [], cartId: null, checkoutUrl: null }) as any,
